...
CTL_SPEC="specs/ledger.yaml"            # original order: store.go 6th, memory.go 7th
ARM_SPEC="specs/ledger-order1.yaml"     # swapped:        memory.go 6th, store.go 7th
TAG="${1:?usage: _stub_order_pair.sh <tag>}"
A_OUT="./generated/ledger-pairA-$TAG"
B_OUT="./generated/ledger-pairB-$TAG"
...
draw () {  # $1 spec  $2 out  $3 label
  ...
  local log="logs/ledger-pair$3-$TAG-$(date +%m%d%H%M).log"
