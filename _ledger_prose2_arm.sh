...
ARM_SPEC="specs/ledger-prose2.yaml"
CTL_OUT="./generated/ledger-ctl-proc3"      # reused, already drawn
ARM_OUT="./generated/ledger-prose2"
...
if ! diff <(sed 's|net/http/httptest, net/http, testing|net/http, net/http/httptest, testing|' "$ARM_SPEC") "$CTL_SPEC" > /dev/null; then
...
LOG="logs/ledger-prose2-$(date +%m%d%H%M).log"
echo "=== ledger INERT-EDIT arm 4 $(date) -> $ARM_OUT (log $LOG) ==="
