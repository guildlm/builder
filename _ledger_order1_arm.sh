...
ARM_SPEC="specs/ledger-order1.yaml"
CTL_OUT="./generated/ledger-ctl-proc3"      # reused, already drawn
ARM_OUT="./generated/ledger-order1"
TARGET="internal/store/memory.go"
WANT_PID=42826
...
import sys
a=open('$CTL_SPEC').read(); b=open('$ARM_SPEC').read()
sys.exit(0 if (sorted(a)==sorted(b) and a!=b) else 1)"; then
  echo "REFUSING: $ARM_SPEC is not a pure reordering of $CTL_SPEC."; exit 7
fi
...
LOG="logs/ledger-order1-$(date +%m%d%H%M).log"
echo "=== ledger ENTRY-ORDER arm 5 $(date) -> $ARM_OUT (log $LOG) ==="
