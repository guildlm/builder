#!/usr/bin/env bash
# LEDGER NAME SWAP 2: the SAME manipulation, a DIFFERENT name, in the SAME file as arm 1.
#
...
ARM_SPEC="specs/ledger-name3.yaml"
CTL_OUT="./generated/ledger-ctl-proc3"      # reused, already drawn
ARM_OUT="./generated/ledger-name3"
TARGET="internal/api/middleware_test.go"
WANT_PID=42826
...
if ! diff <(sed 's/TestMiddlewareRecover/TestRecoverMiddleware/' "$ARM_SPEC") "$CTL_SPEC" > /dev/null; then
  echo "REFUSING: $ARM_SPEC is not $CTL_SPEC with exactly the one swap applied."; exit 7
fi
if [[ $(wc -c < "$CTL_SPEC") -ne $(wc -c < "$ARM_SPEC") ]]; then
  echo "REFUSING: byte counts differ; a transposition cannot change them."; exit 7
fi
...
LOG="logs/ledger-name3-$(date +%m%d%H%M).log"
echo "=== ledger NAME-SWAP-3 arm $(date) -> $ARM_OUT (log $LOG) ==="
...
.venv/bin/python _asdrawn_diff.py "$CTL_OUT" "$ARM_OUT" --target="$TARGET" --rename=TestRecoverMiddleware:TestMiddlewareRecover
echo
