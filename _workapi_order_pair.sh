...
CTL_SPEC="specs/workapi.yaml"            # original order: store.go 6th, memory.go 7th
ARM_SPEC="specs/workapi-order1.yaml"     # swapped:        memory.go 6th, store.go 7th
TAG="${1:?usage: _stub_order_pair.sh <tag>}"
A_OUT="./generated/workapi-pairA-$TAG"
B_OUT="./generated/workapi-pairB-$TAG"
...
draw () {  # $1 spec  $2 out  $3 label
  ...
  local log="logs/workapi-pair$3-$TAG-$(date +%m%d%H%M).log"
