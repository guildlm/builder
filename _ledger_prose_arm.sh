...
ARM_SPEC="specs/ledger-prose1.yaml"
CTL_OUT="./generated/ledger-ctl-proc3"      # reused, already drawn
ARM_OUT="./generated/ledger-prose1"
TARGET="internal/money/money.go"
WANT_PID=42826
...
if ! diff <(sed 's/strings, strconv/strconv, strings/' "$ARM_SPEC") "$CTL_SPEC" > /dev/null; then
...
LOG="logs/ledger-prose1-$(date +%m%d%H%M).log"
echo "=== ledger PROSE-SWAP arm $(date) -> $ARM_OUT (log $LOG) ==="
