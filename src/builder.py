...
def _generate_prompt(
    ...
    test_rule = (
        ...
        "HTTP-TEST HYGIENE: every ServeHTTP call gets its OWN *http.Request, built "
        "by its OWN httptest.NewRequest call, with its OWN freshly-built body. "
        "NEVER hand the same `req` variable to ServeHTTP twice. A request's Body is "
        "an io.Reader and the FIRST ServeHTTP DRAINS it, so the second call sends an "
        "EMPTY body: the handler's json.Decode fails on EOF and answers 400 — so a "
        "'POST twice -> 409' case reports `want 409, got 400` against a handler that "
        "maps ErrExists to 409 perfectly correctly. Refreshing the RECORDER is not "
        "enough and is not the point: `w = httptest.NewRecorder()` followed by "
        "`h.ServeHTTP(w, req)` with the SAME req is exactly the bug. Build the "
        "request again:\n"
        "    req1 := httptest.NewRequest(\"POST\", \"/tasks\", "
        "bytes.NewBufferString(body))\n"
        "    h.ServeHTTP(httptest.NewRecorder(), req1)   // seed it\n"
        "    req2 := httptest.NewRequest(\"POST\", \"/tasks\", "
        "bytes.NewBufferString(body))\n"
        "    w := httptest.NewRecorder(); h.ServeHTTP(w, req2)  // now assert 409\n"
