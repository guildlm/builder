...
ARM_SPEC="specs/ledger-name2.yaml"
CTL_OUT="./generated/ledger-ctl-proc3"      # reused, already drawn
ARM_OUT="./generated/ledger-name2"
...
if ! diff <(sed 's/TestPostAccountUnknown/TestPostUnknownAccount/' "$ARM_SPEC") "$CTL_SPEC" > /dev/null; then
...
LOG="logs/ledger-name2-$(date +%m%d%H%M).log"
echo "=== ledger NAME-SWAP-2 arm $(date) -> $ARM_OUT (log $LOG) ==="
...
.venv/bin/python _asdrawn_diff.py "$CTL_OUT" "$ARM_OUT" --target="$TARGET" --rename=TestPostUnknownAccount:TestPostAccountUnknown
echo
