...
def audit_entry(path: str, body: str) -> dict | None:
    ...
    # AN ENTRY THAT STATES TWO DIFFERENT COUNTS IS BROKEN NO MATTER WHICH ONE IS RIGHT, and
    # the check above cannot say so: `all(c != len(named))` goes quiet the moment ONE of the
    # numbers agrees. Measured on taskapipro's router entry — it opens "WRITE ALL TWELVE ...
    # and ELEVEN test functions" and closes "That is TEN test functions and NO more", ten are
    # enumerated, and this audit called it clean. I wrote the contradiction myself, bumping
    # the header when I added a test and leaving the footer, and then ran the tool built to
    # catch exactly that and was told nothing. The model is handed both sentences.
    excerpts = [body[max(0, m.start() - 80):m.end() + 30].replace("\n", " ").strip()
    ...
    return {"path": path, "stated": counts, "named": named, "listed": listed,
            ...
            "contradiction": sorted(set(counts)) if len(set(counts)) > 1 else [],
            "excerpts": excerpts}
...
"""
    ...
    # THE SHAPE THE MISMATCH CHECK CANNOT SEE: two counts, one of them correct.
    self_contradicting = """
  - path: e_test.go
    purpose: >-
      THREE test functions, all of them:
      TestA: does a thing.
      TestB: does another.
      That is TWO test functions and NO more.
"""
    got = audit_entry("e_test.go", self_contradicting)
    if not got or got["contradiction"] != [2, 3]:
        fails.append(f"an entry stating two different counts must report BOTH: "
                     f"{got and got.get('contradiction')}")
    if got and got["mismatch"]:
        fails.append("this is exactly the case mismatch stays quiet on — if it now fires, "
                     "the contradiction check is being tested through the wrong signal")
    got = audit_entry("a_test.go", ok_spec)
    if got and got["contradiction"]:
        fails.append("one count, stated once, is not a contradiction")
    # REJECT-NOTHING PIN: the same count stated TWICE agrees with itself. An entry may
    # open with the number and close with it, which is a spec written carefully, not a bug.
    twice = """
  - path: f_test.go
    purpose: >-
      TWO test functions, all of them:
      ...
      That is TWO test functions and NO more.
"""
    got = audit_entry("f_test.go", twice)
    if not got or got["contradiction"]:
        fails.append(f"restating the SAME count is agreement, not contradiction: "
                     f"{got and got.get('contradiction')}")
    ...
    for spec in targets:
        for row in audit(spec):
            ...
            if row["contradiction"]:
                contradicting += 1
                print(f"\n{spec.name}  {row['path']}")
                print(f"   SELF-CONTRADICTING COUNT: this one entry states "
                      f"{' and '.join(str(c) for c in row['contradiction'])}.")
                for e in row["excerpts"]:
                    print(f"      ...{e}")
                print(f"      Both sentences go to the model. Whichever is right, one of them "
                      f"is wrong;\n      an additive entry ('THREE MORE, in ADDITION to the "
                      f"five') is the legitimate\n      shape here — read the excerpts before "
                      f"editing.")
            if not row["mismatch"]:
                continue
            flagged += 1
