...
- **`build()`** — the loop: generate every file, run the toolchain, and on
  failure send the offending file(s) back to the coder with the error output for
  a targeted fix, re-running until green or `max_fix_rounds` is hit.
